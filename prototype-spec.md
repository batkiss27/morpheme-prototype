...
| 2026-09-18 | `scoring.threshold` = round(T1 × Π growth(block(k)) × boss × scale); `thresholdGrowthByBlock` replaces `thresholdGrowth`. Substrate levels are `preRun.substrate.handBonus[]` / `freeRedraws[]`. A failed round with a life (or Insurance) reverts to the round's first undo snapshot and CONTINUE returns to ROUND_START of the *same* round. | §5, §7 |
