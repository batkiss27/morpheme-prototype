...
import { BossPreview, CardPanel, ChainView, ConfirmButton, RunHeader, Tile } from '../components';
        ...
        <div>
          <div className="panel">
            ...
            <div className="row" style={{ marginTop: 8 }}>
              <button type="button" disabled={run.undo.length === 0} onClick={() => dispatch({ type: 'UNDO_STEP' })} title="Undoes the last step or card use this round">
                Undo {run.undo.length > 0 ? (run.undo[run.undo.length - 1]!.kind === 'card' ? 'last card' : 'last step') : ''}
              </button>
            </div>
          </div>
          ...
          <div className="panel">
            ...
            <ConfirmButton
              label="Forfeit round"
              confirmLabel={run.lives > 0 ? `Forfeit — lose a life (${run.lives} left)` : 'Forfeit — the run ends'}
              onConfirm={() => dispatch({ type: 'FORFEIT' })}
            />
          </div>
        </div>
      </div>
